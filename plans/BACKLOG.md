...
Implementation checkpoint: the evaluator contract, synthetic software-verification set, arithmetic baseline, labeling guide, candidate-review packet, and candidate-hidden reference-labeling packet are implemented. The labeling packet reads only validated before/after processing outputs, covers the grid with deterministic tiles, omits machine-candidate data, supports keyboard-editable projected coordinates, preserves partial coverage and drafts, and rejects contradictory tile decisions. Candidate-directed and candidate-hidden provisional exports remain distinct from independent adjudicated labels and are not evaluator inputs. The evaluation set and pipeline baseline remain incomplete until Bingham Canyon regions receive documented qualified independent SAR review, deduplication, adjudication, and tuning-set separation is independently confirmed.
