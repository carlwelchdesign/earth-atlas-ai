...
Status: EAT-001 through EAT-011, EAT-DES-001, and EAT-016 are complete. EAT-012 remains in qualified-review and adjudication. EAT-014 has a network-free bundle-to-Ontology projection, a zero-row-safe version 1.3.0 normalized-table package, an authenticated Developer Tier inventory, live raw/media/normalized synthetic imports, five object types linked by six dataset-backed relations, a verified live epoch-millisecond-to-`Timestamp` bridge for Acquisition and Analysis Run, verified ingestion/rendering of a bounded synthetic GeoTIFF, a confirmed negative Map-layer result for the current Ontology, and authenticated Developer Console and Resource Management evidence. Ontology media-reference/capability configuration and raster placement, configured restricted scopes, cleanup behavior, durable visual evidence, and the final decision remain open. The processed real Umbra Bingham Canyon pair is visible in the standalone workbench through the validated, gitignored EAT-016 prepared-demo boundary.
...
- The live Palantir enrollment identifies AIP Developer Tier as the current plan, reports limited compute/users plus a 60-object-type cap and LLM rate limits, and exposes the core data, Ontology, application, code, and AIP tools. Resource Management currently reports zero Foundry compute, storage, and Ontology volume for the last 30 days and separately identifies the Small AIP tier with limited enrollment activity. Exact quota ceilings, EchoAtlas model invocation, and configured restricted scopes remain explicit feasibility gates rather than MVP dependencies.
- The [Palantir feasibility spike](../docs/platform/palantir-feasibility.md) maps and normalizes the portable bundle locally, records live raw/media/normalized import and Ontology behavior including the empty-family boundary, timestamp bridge, and synthetic GeoTIFF ingestion, and separates the remaining map-tiling, cleanup, and restricted-application gates.
