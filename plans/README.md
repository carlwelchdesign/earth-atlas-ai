...
Status: EAT-001 through EAT-011 and EAT-DES-001 are complete. EAT-012 remains in qualified-review and adjudication. EAT-014 has a network-free bundle-to-Ontology projection, a zero-row-safe version 1.3.0 normalized-table package, an authenticated Developer Tier inventory, live raw/media/normalized synthetic imports, five object types linked by six dataset-backed relations, a verified live epoch-millisecond-to-`Timestamp` bridge for Acquisition and Analysis Run, and verified ingestion/rendering of a bounded synthetic GeoTIFF. Ontology-backed raster map tiling, restricted scopes, cleanup evidence, durable evidence, and the final decision remain open. EAT-016 is in progress to expose the already processed real Umbra Bingham Canyon pair in the standalone workbench through a validated, gitignored prepared-demo boundary.
