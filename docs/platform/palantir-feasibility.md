...
Status: **provisional adjust with live synthetic Ontology, raster-ingestion, Developer Console, and usage checkpoints** as of 2026-08-25. EchoAtlas can project a validated analysis bundle into a minimal Palantir-shaped import plan. Carl explicitly approved and completed AIP Developer Tier enrollment, the live plan and application catalog were inspected, and an `EchoAtlas` Foundry project was created. The exact tiny synthetic fixture has been uploaded as raw structured files and PNG evidence media; five object types and six non-empty link types now back that fixture in the live Ontology. A live Pipeline Builder bridge also converts the acquisition and analysis-run epoch-millisecond companions into native `Timestamp` columns. A separately generated synthetic GeoTIFF verifies TIFF media-set ingestion without uploading real Umbra pixels. Authenticated Resource Management and Developer Console views now provide numeric usage and application-boundary evidence. No real Umbra imagery, credentials, API keys, OAuth clients, actions, or applications have been created.
...
The live checkpoint resolves enrollment, plan-name, high-level capacity, application-catalog, raw-file import, PNG and TIFF media-set creation, schema inference for non-empty normalized CSV tables, the minimum object/link mapping, the bounded timestamp transform path, and current enrollment-wide usage reporting. It does not yet prove map tiling from the embedded GeoTIFF georeferencing, a built OSDK/static-hosted application, configured restricted application scopes, per-operation cost attribution, or cleanup behavior.

### Live Developer Console and usage checkpoint

The authenticated Developer Console home reported zero applications and exposed separate **Applications**, **Standalone OAuth clients**, and **Legacy** areas. Its guided **New application** flow exposed four stages: **Basic information**, **Application type**, **Permissions**, and **Review**. The initial form requires a name, a Compass location, and an organization; it states that the application is created in the selected Compass location and inherits permissions from the parent project. This verifies that the enrollment exposes both the application builder and an explicit permissions stage. It does not verify the available restriction granularity because the flow was stopped before any application was created.
...
A subsequent read-only Map checkpoint opened a new unsaved map and inspected **Add to map** without saving it. The **Objects** view reported that it was showing only mappable objects and that no object types matched; the existing five EchoAtlas object types were therefore not available as map layers. The **Overlays** view exposed only the default Day/Night overlay, and searching it for `EchoAtlas` returned no results. This negative result confirms that a retained or previewable GeoTIFF media item does not automatically become a Map overlay. Native tiling and placement require the documented Ontology media-reference/capability path and remain unverified until that resource is explicitly created and tested.
...
3. [ ] Media-set creation, catalog-level model state, normalized object/link resources, the synthetic timestamp transform path, bounded GeoTIFF ingestion, and the current negative Map-layer result are confirmed. Ontology media-reference/capability creation, native tiling/placement, and OSDK/static-hosting configuration remain open.
...
6. [ ] Current usage values and limitations are recorded; durable visual evidence and the final go/adjust/no-go decision remain open.
