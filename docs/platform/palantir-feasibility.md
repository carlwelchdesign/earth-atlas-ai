...
Status: **final adjust decision with live synthetic Ontology, raster placement, restricted application, generated OSDK, private static hosting, usage checkpoints, a verified hosted-client boundary, and a real-derived hosted evidence profile** as of 2026-08-25. EchoAtlas can project a validated analysis bundle into a minimal Palantir-shaped import plan. Carl explicitly approved and completed AIP Developer Tier enrollment, the live plan and application catalog were inspected, and an `EchoAtlas` Foundry project was created. The exact tiny synthetic fixture has been uploaded as raw structured files and PNG evidence media; five domain object types and six non-empty link types now back that fixture in the live Ontology. A live Pipeline Builder bridge also converts the acquisition and analysis-run epoch-millisecond companions into native `Timestamp` columns. A separately generated synthetic GeoTIFF is connected through a sixth, raster-specific Ontology object type and persists as a native Map layer without uploading real Umbra pixels. A restricted public-client application exposes those six object types and six link types through read-only operations, and its generated OSDK is published inside the enrollment. Hosted workbench asset `0.4.0` completes user-approved browser OAuth and a read-only `EchoAtlasAnalysisRun` query while serving the approved Bingham Canyon Umbra-derived before/after evidence and 26 local candidate records. This does not make the imagery Foundry Media Set- or Ontology-backed: raw GeoTIFFs, provider payloads, API keys, client secrets, service users, action types, functions, and write operations remain absent. The closeout evidence is indexed in [Palantir feasibility closeout](../qa/palantir-feasibility-closeout.md).

## Final decision

**Adjust:** retain Palantir as an optional downstream adapter, not the canonical EchoAtlas runtime.

The spike proves useful optional capabilities: normalized bundle import, Ontology relationships, native timestamps, bounded synthetic raster placement, a tightly restricted read-only browser application, OSDK generation, private static hosting, and a truthful real-derived evidence profile. It does not justify moving deterministic geospatial processing, assessment state, or provider policy into Foundry. It also leaves real Media Set/Ontology-backed Umbra imagery, cleanup behavior, exact Developer Tier ceilings/duration, public deployment, scale, and cost unproven. Those are accepted closeout caveats, not silently passed tests.
...
- defer cleanup validation, Foundry Media Set/Ontology-backed real-imagery behavior, and the final go/no-go until the remaining evidence and approval gates are satisfied.
...
Developer Console generated and published `@echoatlas-restricted-synthetic-test/sdk@0.1.0` with generator version `2.58.0`. The package contains the six selected object types and six link types, with zero actions, functions, or interfaces. The hosted UI uses the matching generated object API name through the public OSDK client instead of importing the enrollment-private package into public CI.

### Hosted UI adapter implementation checkpoint

The workbench now contains an optional Palantir provider boundary that activates only on the exact private Foundry-hosted hostname. Every other hostname, including localhost, remains on the standalone path and does not load OAuth or OSDK client code. On the hosted hostname, the client requests only `api:use-ontologies-read` and `api:use-mediasets-read`, uses the registered `/auth/callback`, and performs one read-only, one-row `EchoAtlasAnalysisRun` page probe. The visible provider state is kept separate from bundle provenance: connecting, available, empty, and unavailable states never relabel the locally validated synthetic bundle or imply that Foundry supplied imagery.

The implementation uses the public `@osdk/client` 2.58.0, `@osdk/api` 2.58.0, and `@osdk/oauth` 1.1.1 packages. Its runtime object descriptor mirrors the object API name generated by the enrollment-private package; the private registry package itself is not required by public CI and no personal token is stored. Focused tests verify the non-host boundary, exact callback and scopes, successful read, unavailable fallback, and the unchanged no-Umbra-pixels attribution. The production build splits the provider libraries into lazy chunks, and the staged hosting archive contains 12 hosted files plus two ZIP directory records, the two synthetic SVG fixtures, and no `generated-demo` directory or PNG.

After Carl refreshed the Foundry session, asset `0.3.0` uploaded successfully and Foundry reported 12 hosted files. The preview requested only `api:use-mediasets-read` and `api:use-ontologies-read`, plus standard basic account information and offline access for the browser OAuth session. Carl approved that consent. Preview and production both then reported `Palantir read-only · synthetic run available`, which requires the one-row `EchoAtlasAnalysisRun` query to have completed with a non-empty result. Both surfaces retained the synthetic fixture title, three pending candidates, interpretation boundary, and `no Umbra pixels represented` attribution. Browser inspection found no warnings or errors. Foundry then reported asset `0.3.0` as deployed and live.

Foundry Website Hosting now serves deployed asset version `0.4.0` at `https://echoatlas-restricted-test-teae6zflavlbtz3q.apps.usw-3.palantirfoundry.com/`. Its 472 KB archive contains 16 files and has SHA-256 `275189198bac11fee3463586f1b45d0f4f26d0d42a70e944e54466dfbd14b513`. In addition to the application shell and lazy client chunks, it includes the approved Bingham Canyon Umbra-derived `before.png` and `after.png`, a validated bundle, 26 candidate records, and the explicit synthetic fallback fixtures. Raw GeoTIFFs, provider payloads, credentials, assessment state, and the optional 7.3 MB change-score and 16 MB candidate-overlay previews are excluded; the UI reports that reduced hosted-evidence boundary instead of silently implying completeness. Preview and production browser verification confirmed the real review title and dates, 26 candidates, the central-open-pit-only sensitivity boundary, `Umbra Lab Inc public SAR data · CC-BY-4.0 · derived by EchoAtlas` attribution, both 361×512 images loaded, the separately truthful synthetic Ontology probe status, and zero browser warnings or errors.

The first uploaded asset (`0.1.0`) exposed a static-host fallback edge case: an unknown `/generated-demo/bundle.json` route returned the HTML application shell with status 200, which the client attempted to parse as JSON. The client now treats an HTML shell response as an absent prepared bundle and uses the explicit synthetic fallback, while malformed responses that claim to be JSON still fail closed. Asset `0.2.0` introduced that fix, asset `0.3.0` added the verified read-only OSDK boundary, and deployed asset `0.4.0` adds the bounded real-derived hosted evidence profile.
...
2. [x] Complete the available plan evidence: high-level limits, enabled applications, and current numeric usage are captured; unavailable numeric compute/storage/GPU/user quota ceilings and country/term limits are explicit caveats.
...
4. [x] Generate an application-specific OSDK and deploy the static workbench without coupling the standalone runtime to Palantir. Asset `0.3.0` first completed the user-approved read-only browser OAuth flow and live synthetic analysis-run probe; deployed asset `0.4.0` preserves that boundary while adding the bounded real-derived hosted evidence profile.
5. [x] Configure a restricted test application and record its exact resources and read-only operation scopes.
6. [x] The exact synthetic bundle is present as raw files and media. Non-empty normalized datasets and the live Ontology preserve expected row counts, object identities, typed link endpoints, available-media references, and native acquisition/run timestamps. Empty-family behavior is guarded locally. Deletion/cleanup behavior is explicitly unexercised.
7. [x] Current usage values, limitations, durable resource/deployment evidence, local application imagery evidence, and the final **adjust** decision are recorded.

Real Umbra GeoTIFF ingestion through Media Set/Ontology remains outside the completed spike. The private hosted profile contains only the approved derived PNG evidence and explicit attribution/reduction boundaries.
