...
- defer hosted deployment/runtime validation of the implemented OSDK boundary, cleanup validation, real-imagery behavior, and the final go/no-go until the remaining evidence and approval gates are satisfied.
...
The implementation uses the public `@osdk/client` 2.58.0, `@osdk/api` 2.58.0, and `@osdk/oauth` 1.1.1 packages. Its runtime object descriptor mirrors the object API name generated by the enrollment-private package; the private registry package itself is not required by public CI and no personal token is stored. Focused tests verify the non-host boundary, exact callback and scopes, successful read, unavailable fallback, and the unchanged no-Umbra-pixels attribution. The production build splits the provider libraries into lazy chunks, and the staged hosting archive contains 14 files, the two synthetic SVG fixtures, and no `generated-demo` directory or PNG.

The first approved upload attempt returned HTTP 409 while Foundry reported that the browser session was expiring, followed by a missing `PALANTIR_TOKEN` cookie during reauthentication. No new asset was accepted or deployed. Live OAuth consent, query success, and production hosting therefore remain explicit open evidence gates.
...
4. [x] Generate an application-specific OSDK and deploy a synthetic-only static workbench without coupling the standalone runtime to Palantir. The read-only hosted-client boundary is implemented and locally verified; deployment and live OAuth/query verification of the new build remain open.
