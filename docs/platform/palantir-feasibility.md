...
The live checkpoint resolves enrollment, plan-name, high-level capacity, application-catalog, raw-file import, PNG and TIFF media-set creation, schema inference for non-empty normalized CSV tables, the minimum object/link mapping, the bounded timestamp transform path, native synthetic GeoTIFF placement in Map, and current enrollment-wide usage reporting. It does not yet prove a built OSDK/static-hosted application, configured restricted application scopes, real-imagery behavior, scale or performance characteristics, per-operation cost attribution, or cleanup behavior.
...
4. [ ] Build and verify an OSDK/static-hosted application without coupling the standalone runtime to Palantir.
5. [ ] Configure a restricted test application and record its exact resources and operation scopes.
